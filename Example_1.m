...
tic
[IMF_v5,logM_v5] = Multiv_FIF_v5(S,opts);
toc
...
FIG4=figure;
for ii=1:size(IMF_v5{1},2)-1
    
    plot3(time,IMF_v5{1}(:,ii),IMF_v5{2}(:,ii),'linewidth',2)
    set(gcf,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    x3=xlabel('time') ;
    set(x3,'Interpreter','latex')
    y3=ylabel('H') ;
...
plot3(time,IMF_v5{1}(:,ii),IMF_v5{2}(:,ii),'linewidth',2)
    ...
    %% CASE 2D + time with minimal number of IMFs (max mask length selected via alpha paramter set to 1)
S=[H,D];
opts=Settings_IF_v1('IF.ExtPoints',5,'verbose',0,'IF.alpha',1);
tic
[IMF_v5,logM_v5] = Multiv_FIF_v5(S,opts);
toc
...
ii=14;
plot3(time,sum(IMF_v5{1}(:,14:17),2),sum(IMF_v5{2}(:,14:17),2),'linewidth',2)
    set(gcf,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    x3=xlabel('time') ;
    set(x3,'Interpreter','latex')
    y3=ylabel('H') ;
    ...
    ii=15
    plot3(time,sum(IMF_v5{1}(:,18),2),sum(IMF_v5{2}(:,18),2),'linewidth',2)
    set(gcf,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    x3=xlabel('time') ;
    set(x3,'Interpreter','latex')
    y3=ylabel('H') ;
...
%% CASE 3D + time with minimal number of IMFs (max mask length selected via alpha paramter set to 1)
S=[H,D,Z];
opts=Settings_IF_v1('IF.ExtPoints',5,'verbose',0,'IF.alpha',1);
tic
[IMF_v5,logM_v5] = Multiv_FIF_v5(S,opts);
...
ii=11;
plot3(time,sum(IMF_v5{1}(:,11:15),2),sum(IMF_v5{2}(:,11:15),2),'linewidth',2)
    set(gcf,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    x3=xlabel('time') ;
    set(x3,'Interpreter','latex')
    y3=ylabel('H') ;
    ...
    ii=12;
    plot3(time,sum(IMF_v5{1}(:,16),2),sum(IMF_v5{2}(:,16),2),'linewidth',2)
    set(gcf,'Units', 'Normalized', 'OuterPosition', [0 0 1 1]);
    x3=xlabel('time') ;
    set(x3,'Interpreter','latex')
    y3=ylabel('H') ;
