function [IMF,logM] = Multiv_FIF_v5(f,options,M)

%
%  function IMF = Multiv_FIF_v5(f,options,M)
%
...
%
%   See also SETTINGS_IF_V1, GETMASK_V1, MAXMINS_v3_4.
%
...
if nargin < 1,  help Multiv_FIF_v5; return; end
...
IMF =cell(1,D);
for i=1:D
    IMF{i} = zeros(N,options.IF.NIMFs);
end
...
while countIMFs < options.IF.NIMFs && min(k_pp)>=options.IF.ExtPoints && toc<options.maxTime
    ...
    while SD>options.IF.delta && inStepN < options.IF.MaxInner
        ...
        for i=1:D
            SD(i)=norm(h_ave(:,i))^2/norm(h(:,i))^2;
        end
        SD=max(SD);
    ...
    if options.saveInter==1
        save([nameFile '_intermediate_Multiv_FIF_v5.mat'],'IMF','f','logM','-v7.3');
    end
end %end of while
for i=1:D
...
if options.saveEnd == 1
    save([ 'Final_' nameFile '_Multiv_FIF_v5.mat'],'IMF','logM','-v7.3');
end
